...
excel1_name='bugTagsUserInfos.xls'
excel1_sheet1_name='Sheet1'

excel2_name='usermessage.xls'
...
def read_excel():
    ...
    for i  in range(1,sheet1_row):
        for j in range(1,sheet2_row):
            #sheet1的第i行，第二列的内容
            print('sheet1',int(sheet1_content.cell(i,3).value))
            #sheet2的第j行，第二列的内容
            print('sheet2',int(sheet2_content.cell(j,1).value))
            #比较sheet1和sheet2同列同行的内容，如果相等则将sheet2的内容写入到sheet1中，追加一列
            if int(sheet1_content.cell(i,3).value)==int(sheet2_content.cell(j,1).value):
                # 将moblile写入excel
               # sheet1.write(i,sheet1_col,int(sheet2_content.cell(j,1).value))

                sheet1.write(i, sheet1_col, datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'));
                #将名字写入excel
                sheet1.write(i, sheet1_col+1,sheet2_content.cell(j, 0).value)
                print("ok")

    sheet1.write(0,sheet1_col,'createtime')
    sheet1.write(0, sheet1_col+1, 'bugtagsname')
